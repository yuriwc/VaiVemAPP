...
const App = ({navigation}) => {
  ...
  function getLocation() {
    if (Platform.OS === 'ios') handleGPSPermission();
  ...
  async function postUser() {
    ...
    let dateFinal = `${date.getUTCFullYear()}-${
      date.getUTCMonth() + 1
    }-${date.getDate()}`;
    ...
    if (mensagem == 'Criado com sucesso') {
      ...
      if (mensagem == 'Usuário já existe') {
        ...
        await AsyncStorage.setItem(
          '@latitude',
          '' + response.usuariostatus.latitude,
        );
        await AsyncStorage.setItem(
          '@longitude',
          '' + response.usuariostatus.longitude,
        );
      ...
      <SafeAreaView style={[backgroundStyle, styles.safeContainer]}>
        <View style={[backgroundStyle, styles.container]}>
          {position.latitude === 0 || position.longitude === 0 ? (
            <Button
              title="Conceder Localização"
              onPress={() => getLocation()}
            />
          ) : null}
          <Text
            style={[
              {color: isDarkMode ? Colors.white : Colors.black, marginTop: 150},
            ]}>
          ...
          <DatePicker
            ...
            locale="pt"
          />
          {position.latitude === 0 || position.longitude === 0 ? null : (
